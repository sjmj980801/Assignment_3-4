...
class Question {
    ...
    answer_right = x => this.correct_answer === x;

    shuffled_options=()=>shuffled_list([this.correct_answer,this.option1,this.option2,this.option3]);
}
...
function render_question(round){
    stage_div.innerHTML = "";
    ...
    render_option(current_question, round);
}

function render_option(question, round){
    let options = question.shuffled_options();
    options.forEach(option=>{
        let option_tag = create_element("div", option);
        stage_div.appendChild(option_tag);
        option_tag.addEventListener("click",()=>{
            if(question.answer_right(option_tag.innerText)){
...
function tree_move_up(round){
    ...
    try{
        ...
        if(round.number!==3){
            ...
            stage_div.innerHTML = `🎉Congratulations for our winner, ${greeting} ${name}, You are The Strongest Link, ${bank} is yours now.`;
            won = true;
        }
    }
...
function begin_timer(round){
    let timer = setInterval(()=>{
        if(round.time === 0){
            ...
            clearInterval(timer);
        timer_div.innerText = round.time;
        round.time -= 1;
    },1000);
}
...
const save_div = document.querySelector("aside > *:last-child");
...
save_div.addEventListener("click",save);
